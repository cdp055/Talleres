...
De los tres lenguajes, Scala y Python presentan una sintaxis más simple y expresiva, ya que permiten aplicar funciones directamente sobre las listas sin necesidad de convertirlas en flujos ni definir estructuras adicionales. Java, aunque soporta funciones lambda, sigue siendo más verboso y menos natural para la programación funcional.
...
sum y length son funciones puras.

fromIntegral convierte Int a Double.
...
El código se ejecuta de manera declarativa, enfocándose en qué hacer y no en cómo hacerlo.
...
Al igual que Scala, Python trabaja con listas y tuplas de forma nativa, lo que lo hace ideal para este tipo de tareas funcionales.


Explicación general:
En los tres lenguajes (Java, Scala y Python), el código realiza el mismo proceso pero usando el paradigma funcional, es decir, evitando bucles y variables mutables.
En lugar de usar for o acumuladores, se utilizan funciones de orden superior como map, filter y reduce, que aplican transformaciones y cálculos sobre las listas de forma más concisa y declarativa.
De esta manera, el código resulta más claro, expresivo y cercano al estilo de la programación funcional, donde las operaciones se definen como transformaciones de datos en lugar de instrucciones paso a paso.
```shell
scala-cli repl
```
